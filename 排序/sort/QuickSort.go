...
func (s *QuickSort) pivotIndex(begin, end int) int {
	...
	i := rand.Int()%(end-begin) + begin
	s.Array[begin], s.Array[i] = s.Array[i], s.Array[begin]
	// 备份begin位置的元素
	pivot := s.Array[begin]
	// end指向最后一个元素
	end--
	...
	for begin < end {
		for begin < end { // 先从end处遍历
			if s.cmp(pivot, s.Array[end]) < 0 { // 右边元素 > 轴点元素,end--,继续从end处遍历
				end--
		...
		for begin < end { // 从begin处遍历
			if s.cmp(s.Array[begin], pivot) < 0 { // 轴点元素 > 左边元素,begin++,继续从begin处遍历
				begin++
